# Phase 1: Vault Dev Vertical Slice

## Objective

Prove the smallest complete workspace using `PRODUCT=vault` and `ENVIRONMENT=dev` before creating reusable multi-workspace abstractions. The canonical VM name is `vault-dev`, the canonical workspace path is `vault/dev`, and the target guest architecture is selected to match the Tart host after an explicit compatibility spike.

## Exit Gate

Do not begin Phase 2 until a real `vault-dev` Tart VM boots into Hyprland,
...
- [x] Define executable acceptance tests first for the flake outputs and the resolved constants `vault`, `dev`, `vault-dev`, and `vault/dev`; run them and capture the expected RED result.
...
- [x] Add a minimal `vault/dev` workspace definition and a single NixOS configuration named `vault-dev`; avoid a general workspace framework in this phase.
...
- [x] Add an unmistakable temporary `vault-dev` identity to hostname, wallpaper/background, Quickshell, Ghostty, Starship, and Firefox profile/start surface without introducing the final identity abstraction.
- [x] Write and run a build check for the `vault-dev` NixOS system closure and the Tart-consumable VM/bootstrap artifact.
- [x] Create or import the Tart VM using the deterministic name `vault-dev`, boot it, and verify Hyprland input, display scaling, clipboard policy, audio if needed, and graceful shutdown.
...
- Progress: 20 of 20 implementation tasks complete. The system closure and installer ISO built,
  and a persistent `vault-dev` Tart VM was installed and booted.
...
- Runtime results: hostname is `vault-dev`; the guest is `aarch64`; greetd,
...
- Runtime notes: Tart's NAT DNS proxy did not resolve during installation, so
  ...
  the desktop closure.
- Blocker: the local-only `vault/dev` VPN profile is absent. AWS supports its
