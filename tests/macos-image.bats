...
@test "macOS build wrapper provisions and stops the Packer artifact" {
	...
	grep -Fq '-F /dev/null' "${REPO_ROOT}/macos/scripts/bootstrap-guest-agent"
	grep -Fq 'sshpass -e /usr/bin/ssh "${SSH_OPTIONS[@]}"' "${REPO_ROOT}/macos/scripts/bootstrap-guest-agent"
	grep -Fq 'networksetup -setdnsservers Ethernet 1.1.1.1 8.8.8.8' "${REPO_ROOT}/macos/scripts/bootstrap-guest-agent"
	grep -Fq 'PreferredAuthentications=password' "${REPO_ROOT}/macos/scripts/bootstrap-guest-agent"
	grep -Fq 'PubkeyAuthentication=no' "${REPO_ROOT}/macos/scripts/bootstrap-guest-agent"
	grep -Fq 'IdentitiesOnly=yes' "${REPO_ROOT}/macos/scripts/bootstrap-guest-agent"
}
...
@test "macOS applications receive workspace defaults" {
	...
	grep -Fq 'com.apple.backgroundtaskmanagementagent' "${CONFIGURE_SCRIPT}"
	...
	grep -Fq '"OverrideFirstRunPage": ""' "${CONFIGURE_SCRIPT}"
	grep -Fq '"OverridePostUpdatePage": ""' "${CONFIGURE_SCRIPT}"
	...
	grep -Fq '/Library/PrivilegedHelperTools/com.amazonaws.acvc.helper' "${CONFIGURE_SCRIPT}"
	grep -Fq '/Library/LaunchDaemons/com.amazonaws.acvc.helper.plist' "${CONFIGURE_SCRIPT}"
	grep -Fq 'Contents/Resources/AWS VPN Client/Contents/Library/LaunchServices/com.amazonaws.acvc.helper' "${CONFIGURE_SCRIPT}"
	grep -Fq 'MachServices' "${CONFIGURE_SCRIPT}"
	grep -Fq '/var/log/AWSVPNClient' "${CONFIGURE_SCRIPT}"
	...
	grep -Fq 'vm-vpn.cfg' "${CONFIGURE_SCRIPT}"
	grep -Fq '"Status": "locked"' "${CONFIGURE_SCRIPT}"
