...
@test "macOS applications receive workspace defaults" {
	...
	grep -Fq '"security.webauthn.enable_macos_passkeys": { "Value": false, "Status": "locked" }' "${CONFIGURE_SCRIPT}"
	grep -Fq '"security.webauth.webauthn_enable_softtoken": { "Value": true, "Status": "locked" }' "${CONFIGURE_SCRIPT}"
	grep -Fq '"Value": true' "${CONFIGURE_SCRIPT}"
